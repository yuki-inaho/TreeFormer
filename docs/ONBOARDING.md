...
- node heatmap が点ではなく ridge 状に出る場合は、loss-only ablationの次に `+ablation=heatmap_native_stride4` を使う。このprofileはstride 4 decoder featureからheatmapを直接1x1 projectionし、`DATA.AUX_HEATMAP_TARGET_STRIDE=4`、native-grid sigma 1.0で教師付き学習する。segmentationはdense decoder出力、direction mapは専用towerのままにする。まだridgeが残る場合は `+ablation=heatmap_native_stride4_offset` を使う。これはheatmapを直結のまま保持し、native cellのsub-pixel offsetを別2ch projectionで回帰し、推論時に3x3 NMSとoffsetを適用する。比較軸は `val/heatmap_node_recall`、`val/heatmap_node_precision`、`val/heatmap_duplicate_peak_rate`、`val/heatmap_background_peaks_per_image`、`val/heatmap_peak_contrast`、`val/heatmap_offset_mae`、`val/seg_iou`、`val/paf_masked_l1`。native target用cacheを別に生成する。
...
- graph stage で dense aux を併用する場合は `train=joint_virtual_root_aux` を使う。これは virtual-root forest、root head、graph losses、external mask segmentation、detail boundary、node heatmap、mask-skeleton direction supervisionを同一forwardで有効にし、`MODEL.AUX_HEAD.GRAPH_CONDITIONING=aux_feature`でaux trunk featureをgraph decoderへ渡す。`TRAIN.W_JOINT_AUX`でgraph lossに対するaux lossの寄与を制御する。`checkpoint.save_last=false`を推奨し、Optunaなど多数trialでは`best.pt`とreportのみ残す。
- Optuna tuning は任意 dependency group として管理する。導入は `just install-optuna` または `uv pip install --python "$TREEFORMER_PYTHON" --project . --group tuning`。実行前に `just cache-private-joint-virtual-root-aux-optuna` で探索対象のheatmap sigmaごとのcacheを生成し、その後 `just tune-private-joint-virtual-root-aux` を使う。出力はrepo外 `${TREEFORMER_ASSETS_ROOT}/optuna/joint_virtual_root_aux/` のSQLite study、`trials.csv`、日本語 `report.md`、`best_trial_overrides.yaml`。640x480 joint pilot は mask-skeleton directionを含むcache v3、batch 2、worker 0を安全な既定とし、batchを増やす場合はGPU smokeでVRAMを確認してからoverrideする。reportにはprivate dataset実パスを書かない。
...
- aux supervised stage 完了後は `infer_aux_panel_treeformer.py` / `just infer-aux-panels` で segmentation overlay、必要に応じた detail boundary head、node heatmap、PAF magnitude/direction の validation panel を repo 外へ生成する。4ch checkpoint では detail boundary は既定で非表示、5ch checkpoint では `Pred detail boundary head` として表示する。segmentation-only checkpoint では loss weight 0 の heatmap / PAF panel は既定で非表示にし、未学習出力を評価対象として読まない。node NMS候補とpred heatmap / directionのmaskには推論segmentationのみを使い、GT maskはpanel比較・metric以外に使わない。
