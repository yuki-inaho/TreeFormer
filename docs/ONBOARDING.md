...
- `train=seg_heatmap_paf` は `seg_heatmap` に PAF / edge direction map supervision を追加する。graph output / graph loss は引き続き無効で、`DATA.AUX_TARGET_MODE=seg_heatmap_paf`、`W_AUX_PAF>0` を使う。論文系の direction field を見る段階はこの mode を使い、segmentation-only / heatmap-only の安定性を確認してから進める。
...
- dense aux stage は `FastSegSupervisedDataset` を使う。legacy loader の graph-derived PAF / heatmap 生成を避け、`DATA.AUX_TARGET_MODE` で `seg_only` / `seg_heatmap` / `seg_heatmap_paf` を切り替える。full run 前に repo 外 disk cache を生成し、`seg_only` は `just cache-private-fast-seg`、`seg_heatmap` は `just cache-private-fast-seg-heatmap`、`seg_heatmap_paf` は `just cache-private-fast-seg-heatmap-paf` を使う。`DATA.SEG_CACHE_MODE=disk`、`DATA.NUM_WORKERS=4`、`DATA.PERSISTENT_WORKERS=true` を初期値にする。cache 生成先は `${TREEFORMER_SEG_CACHE_ROOT}` または `${TREEFORMER_ASSETS_ROOT}/cache/fast_seg/...` とし、Git に入れない。
