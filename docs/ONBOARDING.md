...
- dense aux stage は `FastSegSupervisedDataset` を使う。legacy loader の graph-derived PAF / heatmap 生成を避け、`DATA.AUX_TARGET_MODE` で `seg_only` / `seg_heatmap` / `seg_heatmap_paf` を切り替える。full run 前に repo 外 disk cache を生成し、`seg_only` は `just cache-private-fast-seg`、`seg_heatmap` は `just cache-private-fast-seg-heatmap`、`seg_heatmap_paf` は `just cache-private-fast-seg-heatmap-paf` を使う。cache format v3 はdirection target source/encodingを含むため、旧cacheは再利用せず生成し直す。local disk cache の実測では `DATA.NUM_WORKERS=0` が最速だったため、joint graph+aux の pilot / full recipe はこれを既定にする。cache 生成先は `${TREEFORMER_SEG_CACHE_ROOT}` または `${TREEFORMER_ASSETS_ROOT}/cache/fast_seg/...` とし、Git に入れない。
