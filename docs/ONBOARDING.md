...
- Optuna tuning は任意 dependency group として管理する。導入は `just install-optuna` または `uv pip install --python "$TREEFORMER_PYTHON" --project . --group tuning`。実行は `just tune-private-joint-virtual-root-aux` を使い、repo 外 `${TREEFORMER_ASSETS_ROOT}/optuna/joint_virtual_root_aux/` に SQLite study、`trials.csv`、日本語 `report.md`、`best_trial_overrides.yaml` を保存する。report には private dataset 実パスを書かない。
