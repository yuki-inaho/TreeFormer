...
- dense aux stage は `FastSegSupervisedDataset` を使う。legacy loader の graph-derived PAF / heatmap 生成を避け、`DATA.AUX_TARGET_MODE` で `seg_only` / `seg_heatmap` / 将来の PAF 付き mode を切り替える。full run 前に `just cache-private-fast-seg` で repo 外 disk cache を生成し、`DATA.SEG_CACHE_MODE=disk`、`DATA.NUM_WORKERS=4`、`DATA.PERSISTENT_WORKERS=true` を初期値にする。cache 生成先は `${TREEFORMER_SEG_CACHE_ROOT}` または `${TREEFORMER_ASSETS_ROOT}/cache/fast_seg/...` とし、Git に入れない。
