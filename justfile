...
tune-private-joint-virtual-root-aux:
    @test -n "{{private_treeformer_data}}" || (echo "Set TREEFORMER_PRIVATE_DATA to a legacy TreeFormer dataset root" >&2; exit 2)
    @PYTHONPATH=. CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} {{python}} tune_graph_optuna.py --trials "${TREEFORMER_OPTUNA_TRIALS:-20}" --epochs "${TREEFORMER_EPOCHS:-100}" --max-size "${TREEFORMER_MAX_SIZE:-640}" --batch-size "${TREEFORMER_BATCH_SIZE:-8}" --num-workers "${TREEFORMER_NUM_WORKERS:-4}" --train-limit "${TREEFORMER_TRAIN_LIMIT:-null}" --val-limit "${TREEFORMER_VAL_LIMIT:-null}" --output-root "{{private_optuna_joint_vr_aux_output}}" --assets-root "{{assets_root}}" --seg-cache-root "{{seg_cache_root}}" --seg-cache-mode "${TREEFORMER_SEG_CACHE_MODE:-disk}" --python "{{python}}" --cuda-visible-devices "${CUDA_VISIBLE_DEVICES:-0}"
