...
def relation_infer(h, out, model, obj_token, rln_token, nms=False, map_=False):
    ...
    for batch_id in range(h.shape[0]):
        ...
        # 得到误差矩阵
        # accuracy_adj_batch = torch.zeros((node_id.shape[0], node_id.shape[0]))
        # 得到精度矩阵
        ...
        # 计算所有节点对的欧式距离（L2范数）
        # distance_matrix = torch.cdist(out['pred_nodes'][batch_id, node_id, :2].detach(),
        #                               out['pred_nodes'][batch_id, node_id, :2].detach(), p=2)  # 使用 p=1 对应L1范数
        ...
        if node_id.dim() != 0 and node_id.nelement() != 0 and node_id.shape[0] > 1:
            ...
            # cost_adj_batch *= distance_matrix
            # cost_adj_batch *= local_distance_adj_batch
            ...
            # 生成 pred_rel_list
            pred_rel_list = [node_pairs_valid_dict[tuple(sorted((int(xy[0]), int(xy[1]))))] for xy in mst_tree_selected_list if xy[0] != xy[1]]
            ...
            # # 只选择上半矩阵
            # mst_tree_selected_list = []
            # for row in range(mst_adj_batch.shape[0]):
            #     for col in range(mst_adj_batch.shape[1]):
            #         if mst_adj_batch[row, col] != 0:
            #             mst_tree_selected_list.append([row, col])
            # pred_edges.append(torch.tensor(mst_tree_selected_list).cpu().numpy())
            #
            # pred_rel_list = []
            # for xy in mst_tree_selected_list:
            #     x, y = xy
            #     if x != y:
            #         pred_rel_list.append(node_pairs_valid_dict[tuple(xy)])
            # pred_rel = torch.tensor(pred_rel_list).cpu().numpy()
